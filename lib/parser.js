...
_.extend(Parser.prototype, {
  ...
  parse_filters: function (query, opts) {
    // Extract star patterns from opts.
    var patterns = {};
    _.each(opts, function (value, key) {
      if (utils.endsWith(key, '*')) {
        patterns[key.slice(0, -1)] = value;
      }
    });
    ...
    filters = _.filter(filters, function (item) {
      // Matching against plain fields.
      if (_.contains(opts[item[0]], item[1])) {
        return true;
      }

      // Matching against patterns.
      var matches = _.find(patterns, function (value, key) {
        if (utils.startsWith(item[0], key) && _.contains(value, item[1])) {
          return true;
        }
      });
    ...
    // Rearrange filters for ease of use.
